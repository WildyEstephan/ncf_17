...
class HrPayslip(models.Model):
    ...
    total_to_pay = fields.Float(store=True)
        ...
        for slip in self:
            ...
            for loan in slip.employee_id.loan_ids.filtered(lambda loan: loan.state == 'approved'):
                if slip.pay_vacation and slip.vacation_type == 'enjoyed' and 13 <= payslip_day <= 16:
                    loan_lines = loan.loan_line_ids.filtered(
                        lambda loan_line: loan_line.number == loan.next_fee or loan_line.number == loan.next_fee + 1)
                    for l in loan_lines:
                        l.payslip_id = slip.id
                    loan.next_fee += 2 if loan.next_fee < loan.fee_count - 1 else 1
                else:
                    loan_line = loan.loan_line_ids.filtered(
                        lambda loan_line: loan_line.number == loan.next_fee)
    ...
    def _compute_worked_hours(self):
        for register in self.filtered('line_ids'):
            total_amount = register.line_ids.filtered(
                lambda line: line.code == 'NET').total or 0.0
            register.total_to_pay = total_amount

    @api.onchange('employee_id', 'struct_id', 'contract_id', 'date_from', 'date_to')
    def _onchange_employee(self):
        self.input_line_ids = [(0, 0, {'input_type_id': input.id})
                               for input in self.struct_id.input_line_type_ids if input.id not in self.input_line_ids.mapped('input_type_id').ids]
        ...
        extra_hours_15 = sum(working_hours_import_ids.mapped('extra_hours_amount_15'))
        extra_hours_35 = sum(working_hours_import_ids.mapped('extra_hours_amount_35'))
        extra_hours_50 = sum(working_hours_import_ids.mapped('extra_hours_amount_50'))
        holiday_hours = sum(working_hours_import_ids.mapped('holiday_hours_amount'))
        free_days = sum(working_hours_import_ids.mapped('free_days'))

        for input in self.input_line_ids.filtered(lambda ipi: ipi.code in ['HEL', 'HEF', 'HEN', 'HNI', 'PRE', 'HLT']):
            if input.code == 'HEL':
                input.amount = extra_hours_35

            if input.code == 'HLT':
                input.amount = free_days
            if input.code == 'HEN':
                input.amount = extra_hours_50

            if input.code == 'HNI':
                input.amount = extra_hours_15
            if input.code == 'HEF':
    ...
    def _get_last_payslip_vacation(self):
        ...
        if len(payslips_ids) > 1:
            last_slip = self.env['hr.payslip'].browse(
                [payslips_ids[payslips_ids.index(self.id) - 1]])
    ...
    vacation_type = fields.Selection([
        ('enjoyed', 'Vacaciones disfrutadas'),
        ('worked', 'Vacaciones trabajadas'),
        ('unpayed', 'Vacaciones disfrutadas sin adelanto')],
        string="Tipo de vacaciones")
    last_payslip_vacation = fields.Boolean(
        compute='_get_last_payslip_vacation', store=False)

    partial_worked_days = fields.Boolean(string=u"Días trabajados parciales",
                                         help='Usar por ejemplo si un empleado entró a mitad de quincena (No usar si el empleado cobra por hora)')
    ...
    def compute_commission_amount(self, payslip):
        ...
        payments_by_employee = payslip.env['account.payment'].search_read(
            payment_domain, payment_fields)

        payments_amount = sum([p.get('amount') - p.get('amount_tax')
                              for p in payments_by_employee])
    ...
    def compute_sheet(self):
        ...
        for rec in self:
            ...
            real_wage = rec.contract_id.hourly_wage if rec.contract_id.structure_type_id.wage_type == 'hourly' else rec.contract_id.wage
            if rec.contract_id.structure_type_id.wage_type == 'hourly' and rec.real_worked_hours > 0:
                for wdl in rec.worked_days_line_ids:
                    wdl.number_of_hours = rec.real_worked_hours

            if rec.partial_worked_days:
                if rec.contract_id.structure_type_id.wage_type == 'hourly':
                    wage_amount = (
                        real_wage) * sum([line.number_of_days for line in rec.worked_days_line_ids])
                else:
                    wage_amount = (
                        real_wage / 23.83) * sum([line.number_of_days for line in rec.worked_days_line_ids])
                contract_wages.update({rec.contract_id.id: wage_amount})
            if rec.contract_id.structure_type_id.wage_type == 'hourly':
                wage_amount = rec.contract_id.hourly_wage * \
                    sum([line.number_of_hours for line in rec.worked_days_line_ids])
                contract_wages.update({rec.contract_id.id: wage_amount})
            for line in rec.input_line_ids:
                if rec.pay_vacation:
                    if line.code == 'VACA' and line.amount <= 0:
                        raise ValidationError(
                            u"Establezca primero la cantidad de días de vacaciones")
        ...
        for rec in self:
            if rec.pay_vacation and rec.vacation_type == 'enjoyed':
                ...
                for line in rec.line_ids:
                    if line.code == 'VACA':
                        line.amount = (
                            ((wage_amount / 23.83) * vacation_days) - (wage_amount / 2))
                    if line.code == 'NET':
        ...
        for record in self:
            ...
            payslips_ids = [p.id for p in payslips]
            extra_hours = sum(pl.total for pl in record.line_ids if pl.code in [
                              'HEL', 'HEF', 'HEN', 'HNI', 'HLT'])
            ...
            retenciones = sum(l.total for l in record.line_ids.filtered(
                lambda pl: pl.code in ['SFSE', 'SVDSE']))
            if len(payslips_ids) > 1:
                last_slip = record.env['hr.payslip'].browse(
                    [payslips_ids[payslips_ids.index(record.id) - 1]])
                if payslip_day >= 25 and payslip_day <= 31:
