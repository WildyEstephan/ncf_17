...
class HrEmployeeLoan(models.Model):
    ...
    def convert_delta(self, type_id, date):
        if type_id == 'bi-weekly':

            if date.day > 15:
                if date.month == 2:
                    date_new = date.replace(day=28)
                else:
                   date_new = date.replace(day=30)
            else:
                date_new = date.replace(day=15)
        else:
            if date.month == 2:
                date_new = date.replace(day=28)
            else:
    ...
    def compute_draft_lines(self):
        ...
        for i in range(1, self.fee_count + 1):
            ...
            date = self.convert_delta(self.rate_id.type, date)
...
class HrEmployeeLoanLine(models.Model):
    ...
    rate = fields.Float(string="Tasa", digits=(8, 4))
...
class HrEmployeeLoanRate(models.Model):
    ...
    rate = fields.Float(string="Tasa", digits=(8, 5), required=True)
